    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                # начало игры после проигрыша и нажатия нужной кнопки
                player.lives = 3
                delete(enemies, bulletsa, bulletsb)
                counter = 1
                k = 1
            elif event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
                # стрельба при нажатии на любую кнопку мыши или клавиатуры кроме esc
                player.fire()

        # каждые 2 секунды появляется 1-3 объекта: враг или пуля
        # по ходу игры скорость появления увеличивается
        ...
        if player.lives:
            # если есть жизни, обычный игровой цикл
            all_sprites.update()
            ...
            # жизней нет - ожидание решения игрока выйти или начать сначала
